        ...
        <div class="lg:col-span-1">
            <div class="bg-white shadow-md rounded-lg">
                <div class="p-4 space-y-2 border-b">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari NIM atau Nama..." class="border-gray-300 rounded-md shadow-sm w-full">
                </div>

                <div class="max-h-[60vh] overflow-y-auto">
                    @forelse ($krsList as $krs)
                    <div wire:click="selectKrs({{ $krs->id }})" class="p-4 border-b cursor-pointer hover:bg-gray-50
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white shadow-md rounded-lg">
                <div class="p-6 border-b">
                    <h3 class="text-xl font-semibold text-unmaris-blue">{{ $selectedKrs->mahasiswa->nama_lengkap }}</h3>
                    <p class="text-gray-700">{{ $selectedKrs->mahasiswa->nim }} | {{ $selectedKrs->mahasiswa->programStudi->nama_prodi }}</p>
                    <div class="mt-2">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Dosen & Jadwal</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $detail->mataKuliah->kode_mk }} | Kelas {{ $detail->kelas->nama_kelas }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                                        <div>{{ $detail->kelas->dosen?->nama_lengkap ?? 'N/A' }}</div>
                                        <div>{{ $detail->kelas->hari ?? '-' }}, {{ $detail->kelas->jam_mulai ? $detail->kelas->jam_mulai->format('H:i') : '' }}-{{ $detail->kelas->jam_selesai ? $detail->kelas->jam_selai->format('H:i') : '' }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $detail->sks }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="p-6 border-t bg-gray-50 flex justify-end space-x-3">
                    <button wire:click="returnToDraft" class="bg-yellow-500 text-white font-bold py-2 px-4 rounded hover:bg-yellow-600">
                        Kembalikan ke Draft
                    </button>
                    <button wire:click="rejectKrs" class="bg-red-600 text-white font-bold py-2 px-4 rounded hover:bg-red-700">
                        Tolak
                    </button>
                    <button wire:click="approveKrs" class="bg-green-600 text-white font-bold py-2 px-4 rounded hover:bg-green-700">
                        Setujui
                    </button>
                </div>
                @endif
                ...
                <div class="p-6 border-t bg-gray-50 flex justify-end space-x-3">
                    <button wire:click="returnToDraft" class="bg-gray-500 text-white font-bold py-2 px-4 rounded hover:bg-gray-600">
