...
<div>
    <x-slot:header>Manajemen Kategori Berita</x-slot:header>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Post</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($categories as $category)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $category->posts_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button wire:click="openEditModal({{ $category->id }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                            <button wire:click="openDeleteModal({{ $category->id }})" class="text-red-600 hover:text-red-900 ml-4">Hapus</button>
                        </td>
                    </tr>
                @empty
                    <tr>
            ...
            <form wire:submit="save">
                ...
                <div class="mt-4 space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model.live="name" id="name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-unmaris-yellow focus:border-unmaris-yellow">
                        @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label for="slug" class="block text-sm font-medium text-gray-700">URL Slug</label>
                        <input type="text" wire:model="slug" id="slug" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-unmaris-yellow focus:border-unmaris-yellow bg-gray-50">
        ...
        <div class="relative w-full max-w-md p-6 bg-white rounded-lg shadow-lg" @click.stop>
            ...
            <div class="mt-4">
                <p class="text-gray-600">
                    ...
                    <span class="font-bold">{{ $deletingCategory?->name ?? '...' }}</span> ?
                </p>
                <p class="mt-2 text-sm text-red-500">
                    (Catatan: Menghapus kategori tidak akan menghapus berita di dalamnya.)
                </p>
            </div>
            <div class="mt-6 flex justify-end space-x-4">
