        ...
        @foreach ($sliders as $item)
            <div
                ...
                <div class="h-48 bg-gray-100 relative">
                    @if ($item->type == 'video')
                        <video class="w-full h-full object-cover opacity-80" muted loop
                            poster="{{ $item->poster_path ? asset('storage/' . $item->poster_path) : '' }}">
                            <source src="{{ asset('storage/' . $item->file_path) }}" type="video/mp4">
                        </video>
                        <div class="absolute inset-0 flex items-center justify-center">
                            <i class="fas fa-play-circle text-4xl text-white drop-shadow-lg opacity-80"></i>
                        </div>
                        <span
                            class="absolute top-2 right-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded shadow">VIDEO</span>
                    @else
                        <img src="{{ asset('storage/' . $item->file_path) }}" class="w-full h-full object-cover">
                        <span
                            class="absolute top-2 right-2 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded shadow">IMAGE</span>
                    @endif
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="font-bold text-gray-800 line-clamp-1" title="{{ $item->title }}">
                                {{ $item->title ?? 'Tanpa Judul' }}</h4>
                            <p class="text-xs text-gray-500 mt-1 line-clamp-2">
                                {{ $item->description ?? 'Tanpa Deskripsi' }}</p>
                        </div>
                    ...
                    <div class="mt-4 pt-3 border-t flex justify-between items-center">
                        @if ($item->button_text)
                            <span
                                class="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded border border-blue-100 truncate max-w-[150px]">
                        ...
                        <div class="space-x-2">
                            <button wire:click="edit({{ $item->id }})"
                                class="text-gray-400 hover:text-unmaris-blue transition"><i
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" x-data
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div class="px-6 py-4 border-b flex justify-between items-center sticky top-0 bg-white z-10">
                    <h3 class="text-xl font-bold text-gray-800">{{ $isEditing ? 'Edit Slider' : 'Tambah Slider Baru' }}
                    </h3>
                ...
                <form wire:submit="save" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-4">
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="px-4 py-2 rounded-lg border-2 text-center transition peer-checked:border-unmaris-blue peer-checked:bg-blue-50 peer-checked:text-unmaris-blue">
                    ...
                    {{-- Upload Area (IMAGE & VIDEO PREVIEW) --}}
                    <div
                        ...
                        <div wire:loading wire:target="file_upload"
                            class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-20">
                            <svg class="animate-spin h-8 w-8 text-unmaris-blue mb-2" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <span class="text-sm font-bold text-gray-600">Mengupload file...</span>
                        </div>

                        {{-- 2. AREA PREVIEW --}}
                        @if ($file_upload)
                            {{-- Jika ada file BARU yang dipilih (Temporary) --}}
                            <p
                                class="text-green-600 text-sm font-bold mb-2 bg-green-100 inline-block px-2 py-1 rounded">
                                <i class="fas fa-check"></i> File Siap Disimpan</p>

                            @if ($type == 'image')
                                {{-- Preview Gambar Baru --}}
                                <img src="{{ $file_upload->temporaryUrl() }}"
                                    class="h-48 mx-auto rounded shadow object-cover">
                            @elseif($type == 'video')
                                {{-- Preview Video Baru --}}
                                <video controls class="h-48 mx-auto rounded shadow bg-black w-full">
                                    <source src="{{ $file_upload->temporaryUrl() }}" type="video/mp4">
                                    Browser Anda tidak mendukung tag video.
                                </video>
                            @endif
                        @elseif ($old_file_path)
                            {{-- Jika Edit & File Lama Ada --}}
                            <p class="text-gray-500 text-xs mb-2 font-bold uppercase tracking-wider">File Saat Ini:</p>

                            @if ($type == 'image')
                                <img src="{{ asset('storage/' . $old_file_path) }}"
                                    class="h-48 mx-auto rounded shadow object-cover">
                            @else
                                <video controls class="h-48 mx-auto rounded shadow bg-black w-full">
                        ...
                        @else
                            ...
                            <div class="text-gray-400 py-8">
                                ...
                                <p class="text-sm font-medium text-gray-600">Klik untuk pilih
                                    {{ $type == 'video' ? 'Video (MP4)' : 'Gambar (JPG/PNG)' }}</p>
                                <p class="text-xs text-gray-400 mt-1">Maksimal 20MB untuk Video, 2MB untuk Gambar</p>
                            </div>
                        @endif
                        ...
                        @error('file_upload')
                            <div class="absolute bottom-2 left-0 right-0 text-center">
                                <span
                                    class="text-red-500 text-xs bg-red-100 px-2 py-1 rounded font-bold">{{ $message }}</span>
                            </div>
                        @enderror
                    </div>

                    {{-- Khusus Video: Upload Poster --}}
                    @if ($type == 'video')
                        <div class="bg-yellow-50 p-4 rounded-lg border border-yellow-200 relative">
                            <div wire:loading wire:target="poster_upload"
                                class="absolute inset-0 bg-white/50 flex items-center justify-center z-20 rounded-lg">
                                <span class="text-xs font-bold text-yellow-800">Mengupload poster...</span>
                            </div>

                            <label class="block text-sm font-bold text-yellow-800 mb-1">Cover Video (Poster)</label>
                            <p class="text-xs text-yellow-600 mb-3">Gambar ini muncul sebelum video diputar. (Opsional)
                            </p>

                            <div class="flex items-start gap-4">
                                @if ($poster_upload)
                                    <img src="{{ $poster_upload->temporaryUrl() }}"
                                        class="h-20 w-32 object-cover rounded border border-yellow-300 shadow-sm">
                                @elseif ($old_poster_path)
                                    <img src="{{ asset('storage/' . $old_poster_path) }}"
                                        class="h-20 w-32 object-cover rounded border border-yellow-300 shadow-sm">
                                @else
                                    <div
                                        class="h-20 w-32 bg-yellow-100 rounded border border-yellow-200 flex items-center justify-center text-yellow-400">
                                        <i class="fas fa-image text-2xl"></i>
                                    </div>
                                @endif

                                <input type="file" wire:model="poster_upload"
                                    class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-white file:text-yellow-700 hover:file:bg-yellow-100 cursor-pointer">
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold text-gray-700">Judul Utama (Boleh HTML)</label>
                            <input type="text" wire:model="title"
                                placeholder="Contoh: Masa Depan <br> Dimulai Disini"
                                class="w-full border-gray-300 rounded-md shadow-sm mt-1 focus:ring-unmaris-blue focus:border-unmaris-blue">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="2"
                            class="w-full border-gray-300 rounded-md shadow-sm mt-1 focus:ring-unmaris-blue focus:border-unmaris-blue"></textarea>
                    </div>

                    <div
                        class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <div>
                            <label class="block text-sm font-bold text-gray-700">Teks Tombol</label>
                            <input type="text" wire:model="button_text" placeholder="Misal: Daftar Sekarang"
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="button_url" placeholder="Misal: /pmb"
                                class="w-full border-gray-300 rounded-md shadow-sm mt-1">
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t">
                        <button type="button" wire:click="$set('showModal', false)"
                            class="bg-gray-200 px-4 py-2 rounded-lg font-bold hover:bg-gray-300">Batal</button>
                        <button type="submit"
