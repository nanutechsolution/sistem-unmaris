    ...
    <div class="max-w-7xl mx-auto px-6 pb-16 grid lg:grid-cols-3 gap-10 -mt-12 relative z-20">
        ...
        <aside class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-xl border-t-4 border-unmaris-blue lg:sticky lg:top-24">
                ...
                {{-- Form Pencarian --}}
                <form action="{{ route('lpm.announcements.all') }}" method="GET">
                    <div class="relative mb-4">
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Kata kunci judul..." 
                    ...
                    <button type="submit" class="w-full bg-unmaris-blue text-white font-bold py-2.5 rounded-xl hover:bg-unmaris-yellow hover:text-unmaris-blue transition shadow-md text-sm">
                        Terapkan Filter
                    </button>
                    
                    @if(request('q'))
                        <a href="{{ route('lpm.announcements.all') }}" class="block text-center mt-3 text-xs font-bold text-gray-500 hover:text-red-500">
            ...
            <div class="bg-blue-50 p-6 rounded-2xl border border-blue-100 hidden lg:block">
                ...
                <p class="text-sm text-gray-600 mb-4 leading-relaxed">
                    Pantau halaman ini secara berkala untuk mendapatkan informasi jadwal Audit Mutu Internal (AMI) dan Rapat Tinjauan Manajemen (RTM).
                </p>
            </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-xl border border-gray-100">
                ...
                <div class="space-y-6">
                    @forelse($announcements as $ann)
                        <div class="group relative bg-white border border-gray-200 rounded-xl p-5 hover:shadow-lg hover:border-unmaris-blue/30 transition duration-300">
                            <div class="flex flex-col sm:flex-row gap-5">
                                {{-- Date Box Visual --}}
                                <div class="flex-shrink-0 sm:w-20 text-center">
                                    <div class="bg-blue-50 rounded-lg p-2 border border-blue-100 group-hover:bg-unmaris-blue group-hover:border-unmaris-blue transition duration-300">
                                        <span class="block text-2xl font-extrabold text-unmaris-blue group-hover:text-white transition leading-none mb-1">
                                            {{ optional($ann->published_at)->format('d') }}
                                        </span>
                                        <span class="block text-[10px] font-bold text-unmaris-blue/70 uppercase group-hover:text-white/80 transition">
                                ...
                                <div class="flex-grow">
                                    <h3 class="text-lg font-bold text-gray-800 mb-2 group-hover:text-unmaris-blue transition leading-tight">
                                        <a href="{{ route('lpm.announcement', $ann->slug) }}">
                                            {{ $ann->title }}
                                        </a>
                                        {{-- New Badge jika kurang dari 7 hari --}}
                                        @if($ann->published_at && $ann->published_at->diffInDays(now()) < 7)
                                            <span class="ml-2 inline-block bg-red-100 text-red-600 text-[10px] px-2 py-0.5 rounded font-bold align-middle animate-pulse">BARU</span>
                                        @endif
                                    </h3>
                                    ...
                                    <div class="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                                        ...
                                        <a href="{{ route('lpm.announcement', $ann->slug) }}" class="text-sm font-bold text-unmaris-blue hover:text-unmaris-yellow transition flex items-center group-hover/link">
                                            Baca Detail <i class="fas fa-arrow-right ml-1.5 text-xs transform group-hover:translate-x-1 transition"></i>
                                        </a>
                                    </div>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                            <div class="bg-white w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm text-gray-300 text-3xl">
