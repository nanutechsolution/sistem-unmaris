            ...
            <div class="bg-white p-6 rounded-2xl shadow-xl border-t-4 border-unmaris-blue lg:sticky lg:top-24">
                <div class="flex justify-between items-center mb-4">
                    ...
                    @if(request()->hasAny(['kategori_id', 'fakultas_id', 'program_studi_id', 'search']))
                        <a href="{{ route('dokumen.index') }}" class="text-xs text-red-500 font-bold hover:underline">Reset</a>
                    @endif
                </div>
                
                <form action="{{ route('dokumen.index') }}" method="GET" class="space-y-5">
                    
                    {{-- Search --}}
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Pencarian</label>
                        <div class="relative">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Judul dokumen..." 
                    ...
                    <div>
                        ...
                        <select name="kategori_id" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-unmaris-blue transition">
                    ...
                    <div>
                        ...
                        <select name="fakultas_id" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-unmaris-blue transition">
                    ...
                    <div>
                        ...
                        <select name="program_studi_id" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-unmaris-blue transition">
                    ...
                    <button type="submit" class="w-full bg-unmaris-blue text-white font-bold py-2.5 rounded-lg hover:bg-unmaris-yellow hover:text-unmaris-blue transition shadow-md text-sm flex justify-center items-center gap-2">
            ...
            <div class="bg-white p-6 md:p-8 rounded-2xl shadow-xl border border-gray-100 min-h-[500px]">
                ...
                <div class="space-y-4">
                    @forelse($dokumens as $doc)
                        <div class="group bg-white border border-gray-200 rounded-xl p-5 hover:shadow-lg hover:border-unmaris-blue/30 transition duration-300 flex flex-col sm:flex-row items-start gap-4">
                            
                            {{-- Icon File Type --}}
                            <div class="flex-shrink-0">
                                <div class="w-14 h-14 rounded-lg bg-blue-50 text-unmaris-blue flex items-center justify-center text-2xl group-hover:bg-unmaris-blue group-hover:text-white transition duration-300">
                            ...
                            <div class="flex-grow min-w-0">
                                ...
                                <h3 class="text-lg font-bold text-gray-800 mb-1 group-hover:text-unmaris-blue transition leading-tight truncate">
                                    {{ $doc->judul }}
                                </h3>
                                ...
                                <div class="flex items-center text-xs text-gray-400 gap-4">
                                    ...
                                    <span class="flex items-center"><i class="fas fa-download mr-1.5"></i> {{ $doc->download_count }}x Diunduh</span>
                                </div>
                            </div>

                            {{-- Download Button --}}
                            <div class="flex-shrink-0 mt-3 sm:mt-0">
                                <a href="{{ route('dokumen.download', $doc->id) }}" class="inline-flex items-center justify-center w-full sm:w-auto px-5 py-2.5 bg-unmaris-blue text-white font-bold text-sm rounded-lg hover:bg-unmaris-yellow hover:text-unmaris-blue transition shadow-sm">
