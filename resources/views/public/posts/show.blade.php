    ...
    <section class="bg-unmaris-blue text-white pt-32 pb-32 relative overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto px-6 relative z-10 text-center">
             ...
             <nav class="flex justify-center items-center text-sm text-unmaris-yellow/80 mb-6 font-medium space-x-2">
                ...
                <span>/</span>
                @if($post->category)
                    <a href="{{ route('posts.index', ['category' => $post->category->slug]) }}" class="hover:text-white transition">{{ $post->category->name }}</a>
                @else
                    <span>Detail</span>
                @endif
            </nav>
    ...
    <div class="max-w-4xl mx-auto px-6 pb-16 -mt-20 relative z-20">
        <div class="bg-white p-6 md:p-12 rounded-3xl shadow-2xl border border-gray-100">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center border-b border-gray-100 pb-8 mb-8 gap-4"
                 ...
                 @if($post->category)
                    <a href="{{ route('posts.index', ['category' => $post->category->slug]) }}" class="px-4 py-1.5 rounded-full bg-blue-50 text-unmaris-blue font-bold uppercase text-xs tracking-wide hover:bg-unmaris-blue hover:text-white transition">
                        {{ $post->category->name }}
                    </a>
                @else
                    <div></div> {{-- Spacer --}}
                ...
                <div class="flex space-x-2 items-center">
                    ...
                    {{-- Facebook --}}
                    <button @click="shareTo('facebook')" class="w-9 h-9 rounded-full bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-[#1877F2] hover:text-white transition shadow-sm" title="Share to Facebook">
                        <i class="fab fa-facebook-f"></i>
                    </button>
                    
                    {{-- Twitter/X --}}
                    <button @click="shareTo('twitter')" class="w-9 h-9 rounded-full bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-black hover:text-white transition shadow-sm" title="Share to X">
                        <i class="fab fa-twitter"></i>
                    </button>
                    
                    {{-- WhatsApp --}}
                    <button @click="shareTo('whatsapp')" class="w-9 h-9 rounded-full bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-[#25D366] hover:text-white transition shadow-sm" title="Share to WhatsApp">
                        <i class="fab fa-whatsapp"></i>
                    </button>

                    {{-- Copy Link --}}
                    <button @click="copyLink()" class="w-9 h-9 rounded-full bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-gray-600 hover:text-white transition shadow-sm relative" title="Copy Link">
                        <i class="fas fa-link" x-show="!copied"></i>
                        <i class="fas fa-check" x-show="copied" x-cloak></i>
                        
                        {{-- Tooltip Copied --}}
                        <span x-show="copied" x-transition 
                              class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded shadow-lg" x-cloak>
                            Tersalin!
                        </span>
                    </button>
                </div>
            ...
            <article class="prose prose-lg prose-blue max-w-none text-gray-700 leading-relaxed font-serif">
             ...
             <div class="mt-12 pt-8 border-t border-gray-100 flex items-center gap-4 bg-gray-50 p-6 rounded-2xl">
                <div class="w-14 h-14 rounded-full bg-unmaris-blue text-white flex items-center justify-center font-bold text-2xl shadow-md border-2 border-white">
    ...
    <section class="py-20 bg-gray-50 border-t border-gray-200">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                ...
                <a href="{{ route('posts.index') }}" class="inline-flex items-center font-bold text-unmaris-blue hover:text-unmaris-yellow transition">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <a href="{{ route('public.posts.show', $related->slug) }}" class="group flex flex-col h-full bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition duration-300 overflow-hidden border border-gray-100">
                    <div class="h-52 overflow-hidden relative">
                         ...
                         <div class="absolute bottom-4 left-4 text-white text-xs font-bold flex items-center">
                             <i class="far fa-calendar-alt mr-1.5"></i> {{ $related->published_at->format('d M Y') }}
                         </div>
                    </div>
                    <div class="p-6 flex flex-col flex-grow">
                        <h4 class="font-bold text-lg text-gray-800 mb-3 group-hover:text-unmaris-blue transition leading-snug line-clamp-2">
                            {{ $related->title }}
                        </h4>
                        <p class="text-gray-500 text-sm line-clamp-2 mb-4 flex-grow">
                            {!! Str::limit(strip_tags($related->content), 80) !!}
                        </p>
                        <span class="mt-auto text-sm font-bold text-unmaris-blue flex items-center group-hover:underline decoration-unmaris-yellow decoration-2 underline-offset-4">
                            Baca Artikel <i class="fas fa-chevron-right ml-1 text-xs transition-transform group-hover:translate-x-1"></i>
                        </span>
                    </div>
                </a>
                @endforeach
