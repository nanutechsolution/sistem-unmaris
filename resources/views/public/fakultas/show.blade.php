<x-layouts.public :title="$fakultas->nama_fakultas">
    ...
    @php
        ...
        if ($dean) {
            // Gabungkan nama lengkap dan gelar (jika ada)
            $fullName = $dean->nama_lengkap . ($dean->gelar ? ', ' . $dean->gelar : '');

            // Gunakan foto profil jika tersedia, jika tidak buat placeholder dengan inisial
            $photoUrl = $dean->foto_profil
                ? Storage::url($dean->foto_profil)
                : 'https://ui-avatars.com/api/?name=' .
                    urlencode($dean->nama_lengkap) .
                    '&background=0D8ABC&color=fff&size=150';
        } else {
            // Fallback jika tidak ada data dean
            $fullName = 'Pelaksana Tugas Dekan';
            $photoUrl = 'https://ui-avatars.com/api/?name=Pelaksana+Tugas+Dekan&background=0D8ABC&color=fff&size=150';
        }
    ...
    <section class="{{ $bgTheme }} text-white pt-32 pb-24 relative overflow-hidden">
        ...
        <div
            class="max-w-7xl mx-auto px-6 relative z-10 text-center md:text-left flex flex-col md:flex-row items-center gap-8">
            {{-- Icon Besar --}}
            <div
                class="w-24 h-24 md:w-32 md:h-32 bg-white/10 backdrop-blur-md rounded-3xl flex items-center justify-center text-5xl md:text-6xl text-unmaris-yellow shadow-2xl border border-white/20 transform rotate-3">
                <i class="{{ $icon }}"></i>
            </div>

            <div>
                <span
                    class="inline-block py-1 px-3 rounded-full bg-white/20 text-white text-xs font-bold tracking-widest uppercase mb-3 border border-white/30">
                    Fakultas Akademik
                </span>
            ...
            <aside class="lg:col-span-1 space-y-8 order-2 lg:order-1">
                ...
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden border-t-4 border-unmaris-yellow">
                    <div class="p-6 text-center bg-gray-50 border-b border-gray-100">
                        <div
                            class="w-24 h-24 mx-auto rounded-full bg-gray-300 border-4 border-white shadow-md overflow-hidden mb-4">
                            {{-- Foto Dekan Placeholder --}}
                            <img src="{{ $photoUrl }}" alt="{{ $fullName }}"
                                class="w-full h-full object-cover rounded-full"
                                onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name=Unknown&background=0D8ABC&color=fff&size=150';" />
                        </div>
                    ...
                    <div class="p-6">
                        <p class="text-gray-600 text-sm italic text-center leading-relaxed">
                            "Selamat datang di {{ $fakultas->nama_fakultas }}. Kami berkomitmen memberikan pendidikan
                            terbaik berbasis karakter dan teknologi."
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-unmaris-blue rounded-2xl p-6 text-center text-white relative overflow-hidden">
                    ...
                    <a href="/pmb"
                        class="inline-block w-full py-3 bg-unmaris-yellow text-unmaris-blue font-bold rounded-lg hover:bg-white transition relative z-10 shadow-lg">
                        Daftar Sekarang
                    </a>
                </div>
            </aside>
            ...
            <div class="lg:col-span-2 order-1 lg:order-2">
                <div class="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 min-h-[500px]">
                    <h2 class="text-2xl font-extrabold text-gray-800 mb-6 flex items-center">
                        <span
                            class="bg-blue-50 text-unmaris-blue w-10 h-10 rounded-full flex items-center justify-center mr-3 text-lg">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        @forelse($fakultas->programStudis as $prodi)
                            <div
                                class="group relative border border-gray-200 rounded-xl p-5 hover:shadow-lg hover:border-unmaris-blue/50 transition duration-300 bg-white">
                                {{-- Badge Jenjang --}}
                                <span
                                    class="absolute top-4 right-4 bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-1 rounded uppercase group-hover:bg-unmaris-blue group-hover:text-white transition">
                                    {{ $prodi->jenjang }}
                                </span>

                                <div
                                    class="w-12 h-12 rounded-lg bg-blue-50 text-unmaris-blue flex items-center justify-center text-xl mb-4 group-hover:scale-110 transition">
                                    <i class="fas fa-graduation-cap"></i>
                                </div>

                                <h3
                                    class="text-lg font-bold text-gray-800 mb-2 group-hover:text-unmaris-blue transition">
                                    {{ $prodi->nama_prodi }}
                                </h3>
                                ...
                                <div class="space-y-1 mb-4">
                                    <p class="text-xs text-gray-500 flex items-center">
                                        ...
                                        Akreditasi: <span
                                            class="font-bold ml-1 text-gray-700">{{ $prodi->akreditasi ?? 'Proses' }}</span>
                                    </p>
                                    <p class="text-xs text-gray-500 flex items-center">
                        ...
                        @empty
                            <div
                                class="col-span-2 text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                ...
                <div class="mt-8 bg-blue-50 rounded-2xl p-8 border border-blue-100">
                    ...
                    <p class="text-gray-700 text-sm leading-relaxed">
                        {{ $fakultas->nama_fakultas }} UNMARIS dirancang untuk memberikan pengalaman belajar yang
                        relevan dengan industri. Kurikulum kami diperbarui secara berkala dan didukung oleh dosen
                        praktisi serta akademisi berpengalaman.
                    </p>
                </div>
            </div>
