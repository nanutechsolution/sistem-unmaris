<x-layouts.public title="Beranda — Universitas Maritim">
    ...
    <section class="py-24 bg-gray-50 relative">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[180px]">
                ...
                {{-- Card 1: Akreditasi --}}
                <div class="md:col-span-2 md:row-span-2 bg-unmaris-blue rounded-3xl p-8 text-white flex flex-col justify-between relative overflow-hidden group hover:shadow-2xl transition duration-500">
                    <div class="absolute top-0 right-0 w-80 h-80 bg-unmaris-yellow/10 rounded-full blur-3xl -mr-20 -mt-20 transition duration-700 group-hover:scale-110"></div>
                    <div class="relative z-10">
                        <div class="bg-white/10 w-fit px-3 py-1 rounded-lg text-xs font-bold mb-4 backdrop-blur-md border border-white/10">INSTITUSI</div>
                        <h3 class="text-xl opacity-90">Terakreditasi Nasional</h3>
                        <p class="text-5xl md:text-6xl font-extrabold mt-2 tracking-tight">
                            BAIK <br> <span class="text-unmaris-yellow">SEKALI</span>
                        </p>
                    </div>
                    <div class="relative z-10 flex justify-between items-end">
                        <p class="text-sm text-blue-200 max-w-xs">Diakui oleh BAN-PT sebagai institusi pendidikan berkualitas tinggi.</p>
                        <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center group-hover:bg-unmaris-yellow group-hover:text-unmaris-blue transition duration-300">
    ...
    <section class="py-24 bg-white relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 relative z-10">
            <div class="text-center max-w-3xl mx-auto mb-16">
                <h2 class="text-4xl font-extrabold text-gray-900 mb-4">Temukan <span class="text-unmaris-blue">Passion</span>-mu</h2>
                <p class="text-gray-600 text-lg">Jelajahi {{ $totalProdi }} Program Studi yang tersebar di {{ $totalFakultas }} Fakultas unggulan kami.</p>
            </div>

            <div class="grid md:grid-cols-3 gap-8">

                @foreach($faculties as $fakultas)
                {{-- Logika Warna & Icon berdasarkan nama Fakultas (Bisa disesuaikan) --}}
                @php
                $icon = 'fas fa-university'; // Default
                ...
                } elseif(Str::contains($fakultas->nama_fakultas, ['Pertanian', 'Sains'], true)) {
                $icon = 'fas fa-leaf';
                $colorClass = 'text-green-600 bg-green-100 group-hover:bg-green-600 group-hover:text-white';
                }
                @endphp

                <div class="group relative bg-white rounded-2xl p-8 border border-gray-100 shadow-lg hover:shadow-2xl transition duration-300 hover:-translate-y-2 flex flex-col h-full">
                    <div class="w-14 h-14 {{ $colorClass }} rounded-xl flex items-center justify-center text-2xl mb-6 transition">
                        <i class="{{ $icon }}"></i>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-800 mb-3 group-hover:text-unmaris-blue transition">{{ $fakultas->nama_fakultas }}</h3>
                    ...
                    <a href="#" class="inline-flex items-center text-unmaris-blue font-bold text-sm group-hover:text-unmaris-yellow transition mt-auto">
